// netlify/functions/exercise-media.js
...
exports.handler = async function (event) {
  ...
  const excludeId = event.queryStringParameters?.exclude || null;
  if (!name) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing name parameter' }) };
  ...
  function aliasesFor(normalisedQuery) {
    const out = [];
  ...
  function fallbackVariants(normalisedQuery) {
  ...
  if (!bestEx || bestScore < MIN_ACCEPT_SCORE) {
    return { statusCode: 404, headers, body: JSON.stringify({ error: 'Exercise not found', name, bestScore }) };
  }
