// netlify/functions/notify-ai-coach.js
// Scheduled: fires at both UTC equivalents of 08:00 Europe/London (see
// londonNow() in _lib/webpush.js for why), an hour after the plain
// readiness push from notify-daily-summary.js so overnight Health sync
...
const { computeRecoveryScore, computeSleepScore, computeStrainScore } = require('./_lib/health-scores');
const { fetchNightscoutInput } = require('./notify-glucose-forecast');
...
async function fetchRecentInsulinGap(userId) {
...
function consumedFor(dayLog, healthRow) {
...
function buildWeekSummary({ weekStart, todayStr, dailyLogs, healthRows, foodLogRows, sessions }) {
  const healthByDate = {}; (healthRows || []).forEach(h => { healthByDate[h.log_date] = h; });
  const logByDate = {}; (dailyLogs || []).forEach(l => { logByDate[l.log_date] = l; });
  const foodByDate = {};
  (foodLogRows || []).forEach(r => { foodByDate[r.log_date] = (foodByDate[r.log_date] || 0) + (Number(r.calories_kcal) || 0); });
  ...
  const sessionLines = (sessions || []).map(s => {
    ...
    const setCount = exercises.reduce((n, ex) => n + (ex.workout_sets || []).length, 0);
    const volume = exercises.reduce((v, ex) => v + (ex.workout_sets || []).reduce((sv, st) => sv + (Number(st.weight) || 0) * (Number(st.reps) || 0), 0), 0);
...
function insightLines(insights, limit) {
...
function buildContextSections({ goal, recovery, sleep, strain, week, diabetes }) {
  const sections = [];
...
function buildPrompt({ profileLabel, goal, recovery, sleep, strain, week, diabetes }) {
...
module.exports.buildWeekSummary = buildWeekSummary;
module.exports.buildContextSections = buildContextSections;
module.exports.buildPrompt = buildPrompt;
module.exports.fetchRecentInsulinGap = fetchRecentInsulinGap;

exports.handler = async function () {
  const now = londonNow();
  if (now.hour !== 8) return { statusCode: 200, body: 'not 08:00 London — skipping' };
  if (!SB_URL || !SB_SERVICE) return { statusCode: 500, body: 'Supabase env vars missing' };
  if (!ANTHROPIC_API_KEY) return { statusCode: 500, body: 'ANTHROPIC_API_KEY not configured' };
  ...
  if (!profiles?.length) return { statusCode: 200, body: 'no profiles' };
  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      console.error(`notify-ai-coach failed for user ${userId}:`, err.message);
      errors++;
    }
  }

  return { statusCode: 200, body: JSON.stringify({ generated, sent, failed, errors }) };
};
