...
//
// Ported from src/netlify/functions/health-sync.js — mechanical
// translation to Deno.serve; logic unchanged. Node's crypto.createHash
// is replaced with Web Crypto's crypto.subtle.digest for the SHA-256 API
// key hash, same pattern as the other Nightscout-facing ports.
...
Deno.serve(async (req: Request) => {
  ...
  if (!keyRes.ok || !(keyRes.data as any[])?.length) {
    return new Response(JSON.stringify({ error: 'Invalid API key' }), { status: 401, headers: HEADERS });
  }

  const { user_id, id: keyId } = (keyRes.data as any[])[0];
  ...
  const ignoreWeight = profRes.ok && (profRes.data as any[])?.[0]?.manual_weight_logging === true;
  ...
  if (!metrics.length && !workouts.length) {
    return new Response(JSON.stringify({ received: 0, processed: 0, message: 'No metrics or workouts in payload' }), { status: 200, headers: HEADERS });
  }
  ...
  const activitySamples: { steps: { t: number; qty: number }[]; hr: { t: number; val: number }[] } = { steps: [], hr: [] };
  ...
  for (const w of workouts) {
    ...
    if (res.ok) {
      ...
      // down only ever flags a window that had NO matching workout at
      // the time it ran — a workout logged (or synced) afterward for
      // that same span shouldn't leave a stale confirm/dismiss prompt
      ...
      if (overlapRes.ok && (overlapRes.data as any[])?.length) {
        for (const d of overlapRes.data as any[]) {
    ...
    if (Array.isArray(w.heartRateRecovery) && w.heartRateRecovery.length >= 2) {
      const samples = w.heartRateRecovery.map((p: any) => Number(p.qty)).filter((n: number) => Number.isFinite(n));
      if (samples.length >= 2) {
  ...
  if (!metrics.length && !Object.keys(byDate).length) {
    return new Response(JSON.stringify({ received: 0, processed: 0, workoutsProcessed, message: 'No metrics in payload' }), { status: 200, headers: HEADERS });
  }
  ...
  for (const metric of metrics) {
    ...
    for (const item of dataPoints) {
      ...
      const toKcal = (val: number, units: string) => {
  ...
  if (activitySamples.steps.length) {
    try {
      const restingHrFallback = Object.values(byDate).map((d: any) => d.resting_hr).find(v => v != null)
        ?? await fetchRecentRestingHr(user_id);
      const weightKgFallback = Object.values(byDate).map((d: any) => d.weight_kg).find(v => v != null)
      ...
      if (windows.length) {
        ...
        const existingWorkouts = existingWorkoutsRes.ok ? ((existingWorkoutsRes.data as any[]) || []) : [];
        const existingDetected = existingDetectedRes.ok ? ((existingDetectedRes.data as any[]) || []) : [];
        ...
        const findOverlappingWorkout = (w: any) => existingWorkouts.find((rw: any) => {
          const rs = new Date(rw.started_at).getTime(), re = new Date(rw.ended_at).getTime();
          return w.startMs < re && w.endMs > rs;
        });
        const findDuplicateDetected = (w: any) => existingDetected.find((ed: any) =>
      ...
      console.error('Activity detection error:', (err as Error).message);
    }
  }
  ...
  for (const [logDate, d] of Object.entries(byDate) as [string, any][]) {
    ...
    if (d._glucose?.length) {
      d.glucose_avg_mmol = round2(d._glucose.reduce((a: number, b: number) => a + b, 0) / d._glucose.length);
    ...
    // Aggregate SpO2
    if (d._spo2?.length) {
      d.spo2_avg = round1(d._spo2.reduce((a: number, b: number) => a + b, 0) / d._spo2.length);
    ...
    if (d._hrSamples?.length) {
      d.heart_rate_avg = round1(d._hrSamples.reduce((a: number, b: number) => a + b, 0) / d._hrSamples.length);
      delete d._hrSamples;
    }
    ...
    if (res.ok) {
      ...
      const logUpdate: any = { user_id, log_date: logDate };
      let hasUpdate = false;
...
function computeReadiness(d: any) {
...
async function fetchRecentRestingHr(user_id: string) {
  ...
  return res.ok && (res.data as any[])?.[0]?.resting_hr != null ? Number((res.data as any[])[0].resting_hr) : null;
}

async function fetchRecentWeightKg(user_id: string) {
  ...
  return res.ok && (res.data as any[])?.[0]?.weight_kg != null ? Number((res.data as any[])[0].weight_kg) : null;
}
...
function stepsToCalories(steps: number, weightKg: number | null) {
...
function detectUndetectedActivity(
  stepSamples: { t: number; qty: number }[],
  hrSamples: { t: number; val: number }[],
  restingHr: number | null
) {
  const binMs = ACTIVITY_BIN_MIN * 60000;
  const binOf = (t: number) => Math.floor(t / binMs);
  const bins = new Map<number, { steps: number; hrSum: number; hrCount: number; maxHr: number }>();
  function getBin(idx: number) {
  ...
  const windows: { startMs: number; endMs: number; durationMin: number; steps: number; avgHeartRate: number | null; maxHeartRate: number | null }[] = [];
...
async function notifyDetectedActivities(user_id: string, windows: any[]) {
  const subsRes = await sbFetch(`/rest/v1/push_subscriptions?user_id=eq.${user_id}&select=endpoint,p256dh,auth_key`);
  const subs = subsRes.ok ? ((subsRes.data as any[]) || []) : [];
  ...
  for (const s of subs) {
    try { await sendWebPush({ endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.auth_key } }, payload); } catch { /* best-effort */ }
  }
}
...
function qtyToKcal(field: { qty?: number; units?: string } | undefined) {
...
function qtyToKm(field: { qty?: number; units?: string } | undefined) {
  if (field?.qty == null) return null;
  const val = Number(field.qty);
  const units = (field.units || '').toLowerCase();
...
async function sbFetch(path: string, method = 'GET', body: unknown = null, extra: Record<string, string> = {}) {
  try {
...
export { detectUndetectedActivity, stepsToCalories };
